...
// 1) Force all sensors to clear, so no new walls appear
front_echo_cycles = ECHO_500MM;
left_echo_cycles  = ECHO_500MM;
right_echo_cycles = ECHO_500MM;

// 2) Wait until we return to FORWARD (state 0)
wait_for_state(4'd0, 32'd1_300_000);   // ~2 sensor periods

// 3) Do NOT add an extra long wait here – check immediately
if (LED[3:0] === 4'd0)
...
// 4) Motor pins should now correspond to FORWARD
